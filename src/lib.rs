...
enum XValue<'a> { Null, Int(i32), Str(Rc<String>), Pair(*mut Pair<'a>) }
...
pub struct PairRoot<'a>(*mut Pair<'a>, *const Heap<'a>);
...
impl<'a> PairRoot<'a> {
    unsafe fn new(heap: &Heap<'a>, p: *mut Pair<'a>) -> PairRoot<'a> {
        heap.pin(p);
        PairRoot(p, heap as *const Heap<'a>)
    }
...
fn value_to_heap<'a>(v: Value<'a>) -> XValue<'a> {
    match v {
        ...
        Value::Pair(PairRoot(p, _)) => XValue::Pair(p)
    }
}
...
unsafe fn value_from_heap<'a>(heap: &Heap<'a>, v: &XValue<'a>) -> Value<'a> {
    match v {
        ...
        &XValue::Pair(p) => Value::Pair(PairRoot::new(heap, p))
    }
}
...
mod test {
    ...
    fn bug_swapping() {
        ...
        let obj2 = heap2.alloc((Value::Null, Value::Pair(obj1)));  // should not be allowed
        heap1.force_gc();
        heap2.force_gc();
    }
